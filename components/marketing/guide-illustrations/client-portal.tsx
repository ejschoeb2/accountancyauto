...
import { Upload, FileCheck, ShieldCheck, File, Globe } from "lucide-react";
...
const CHECKLIST = [
  ...
  { label: "SA302",             delay: 0.40 },
];

const DROPPING_FILES = [
  { name: "P60_2025.pdf",          delay: 0.7 },
  { name: "barclays_stmt_q4.pdf",  delay: 1.2 },
  { name: "dividends_2025.pdf",    delay: 1.7 },
];

export const ClientPortalIllustration = ({ isHovered }: { isHovered: boolean }) => {
  ...
  useEffect(() => {
    ...
    if (isHovered) {
      setChecked(new Set());
      const t0 = setTimeout(() => setChecked(new Set([0])), 1000);
      const t1 = setTimeout(() => setChecked(new Set([0, 1])), 1500);
      const t2 = setTimeout(() => setChecked(new Set([0, 1, 2])), 2000);
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-5 py-4 select-none overflow-hidden">
      {/* Portal URL bar */}
      <motion.div
        className="flex items-center gap-1.5 rounded-lg border border-border/30 bg-background/80 px-3 py-[5px] mb-3 w-full max-w-[260px]"
        animate={{ opacity: isHovered ? 1 : 0.3 }}
        transition={{ duration: 0.3 }}
      >
        <Globe size={9} className="text-emerald-500 flex-shrink-0" strokeWidth={2} />
        <span className="text-[8px] text-muted-foreground/50 truncate">
          app.prompt.so/portal/abc123
        </span>
      </motion.div>

      <div className="w-full max-w-[260px] flex gap-4">
        {/* Left: upload zone + files */}
        <div className="flex-1">
          <motion.div
            className="flex flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-violet-400/30 bg-violet-500/[0.03] px-3 py-4 mb-2.5"
            animate={{
            ...
            {CHECKLIST.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-[8px] leading-none ${isDone ? "text-violet-400 line-through" : "text-muted-foreground/50"}`}
                  >
                    {item.label}
                  </span>
                </motion.div>
